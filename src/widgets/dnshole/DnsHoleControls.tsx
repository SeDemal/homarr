...
function DnsHoleControlsWidgetTile({ widget }: DnsHoleControlsWidgetProps) {
  ...
  const toggleDns = async (action:"enable"|"disable") => {
    if (!sessionData?.user?.isAdmin) {return Promise.reject(new Error('Forbidden'));}
    await mutateAsync(
      {
        action,
        configName,
  ...
  return (
    <Stack justify="space-between" h={'100%'} spacing="0.25rem">
      ...
      <Stack
        ...
        style={{
          flex: '1',
          justifyContent: sessionData?.user?.isAdmin && widget.properties.showToggleAllButtons ? 'flex-end' : 'space-evenly',
        }}
      >
